...
def browseFile():
    ...
    blank_img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\output.png", IMG_WIDTH, IMG_HEIGHT)
...
def copy_move_forgery():
    ...
    if forgery is None:
        ...
        img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\no_copy_move.png", IMG_WIDTH, IMG_HEIGHT)
        ...
        img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\copy_move.png", IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
...
def metadata_analysis():
    ...
    if img_exif is None:
        ...
        # Retrieve the output image and display in resultPanel
        img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\no_metadata.png", IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
        ...
        # Retrieve the thumbs up image and display in resultPanel
        img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\metadata.png", IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
...
def noise_variance_inconsistency():
    ...
    if(noise_forgery):
        ...
        # Retrieve the output image and display in resultPanel
        img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\varience.png", IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
        ...
        # Retrieve the thumbs up image and display in resultPanel
        img = getImage(r"C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\no_varience.png", IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
...
def cfa_artifact():
    ...
    if(identical_regions_cfa):
        # Retrieve the output image and display in resultPanel
        img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images/cfa.png', IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
        ...
        # print('\nSingle compressed')
        # Retrieve the thumbs up image and display in resultPanel
        img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\no_cfa.png', IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
...
def jpeg_Compression():
    ...
    if(double_compressed):
        ...
        # Retrieve the output image and display in resultPanel
        img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\double_compression.png', IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
        ...
        # print('\nSingle compressed')
        # Retrieve the thumbs up image and display in resultPanel
        img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\single_compression.png', IMG_WIDTH, IMG_HEIGHT)
        resultPanel.configure(image=img)
        resultPanel.image = img
...
root = Tk()
root.config(bg='#E2BFB3')
root.title("Image Forgery detection")
root.geometry(f"{1600}x{850}")

root.grid_columnconfigure(1, weight=3)
root.grid_rowconfigure((0, 1, 2), weight=1)
root.sidebar_frame = customtkinter.CTkFrame(root, width=140, corner_radius=0)
root.sidebar_frame.grid(row=0, column=0, rowspan=3, sticky="nsew")
root.sidebar_frame.grid_rowconfigure(7, weight=1)
root.logo_label = customtkinter.CTkLabel(root.sidebar_frame, text="Detection Methods", font=customtkinter.CTkFont(size=20, weight="bold"))
root.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

root.iconbitmap(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\favicon.ico')
...
root.image_frame = customtkinter.CTkFrame(root, width=140, corner_radius=0)

root.image_frame.grid(row=0, column=1, rowspan=3, padx=20, pady=20, sticky="nsew")
# Label for the results of scan
resultLabel = Label(root.image_frame,text="IMAGE FORGERY DETECTOR", font=("Nexa", 20), bg=('black'), foreground=('blue'))
#resultLabel.grid(row=0, column=0)#, columnspan=3)
resultLabel.grid(row=0, column=1, padx=5, pady=10)

# Get the blank image
input_img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\input.png', IMG_WIDTH, IMG_HEIGHT)
#middle_img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\middle.png', IMG_WIDTH, IMG_HEIGHT)
output_img = getImage(r'C:\Users\VIKASH GAUTAM\Downloads\forgory\ppp\images\output.png', IMG_WIDTH, IMG_HEIGHT)

# Displays the input image
imagePanel = Label(root.image_frame,image=input_img)
#customtkinter.CTkLabel(root.image_frame, image= input_img)
imagePanel.image = input_img
imagePanel.grid(row=3, column=0, padx=5, pady=10)
...
resultPanel = Label(root.image_frame,image=output_img)
resultPanel.image = output_img
resultPanel.grid(row=3, column=2, padx=5, pady=10)

# Label to display the path of the input image
fileLabel = Label(root.image_frame,text="No file selected", fg="grey", font=("Times", 10))
fileLabel.grid(row=4, column=1)
# fileLabel.grid(row=2, column=0, columnspan=2)


# Progress bar
progressBar = ttk.Progressbar(root.image_frame,length=400)
...
s = ttk.Style()
s.configure('my.TButton', font=('Nexa', 12,))

# Button to upload images
uploadButton = ttk.Button(root.image_frame,
    text="Upload Image", style="my.TButton", command=browseFile,)
uploadButton.grid(row=6, column=1, sticky="nsew", pady=5)
# uploadButton.grid(row=4, column=0, columnspan=2, sticky="nsew", pady=5)

# Button to run the Compression detection algorithm
compression = ttk.Button(root.sidebar_frame,text="Compression-Detection",
                         style="my.TButton", command=jpeg_Compression)
compression.grid(row=1, column=0, columnspan=2, pady=20)
# startButton.grid(row=5, column=0, columnspan=2, sticky="nsew", pady=5)

# Button to run the Metadata-Analysis detection algorithm
metadata = ttk.Button(root.sidebar_frame,text="Metadata-Analysis",
                      style="my.TButton", command=metadata_analysis)
metadata.grid(row=2, column=0, columnspan=2, pady=20)

# Button to run the CFA-artifact detection algorithm
artifact = ttk.Button(root.sidebar_frame,text="CFA-artifact detection",
                      style="my.TButton", command=cfa_artifact)
artifact.grid(row=3, column=0, columnspan=2, pady=20)

# Button to run the noise variance inconsistency detection algorithm
noise = ttk.Button(root.sidebar_frame,text="Noise-Inconsistency",
                   style="my.TButton", command=noise_variance_inconsistency)
noise.grid(row=4, column=0, columnspan=2, pady=5)

# Button to run the Copy-Move  detection algorithm
copy_move = ttk.Button(root.sidebar_frame,text="Copy-Move", style="my.TButton", command=copy_move_forgery)
copy_move.grid(row=5, column=0, columnspan=2, pady=20)

# Button to run the E
ela = ttk.Button(root.sidebar_frame,text="Error-Level Analysis", style="my.TButton", command=ela_analysis)
ela.grid(row=6, column=0, columnspan=2, pady=20)

# Button to run the Image pixel Analysis algorithm
image_stegnography = ttk.Button(root.sidebar_frame,text="Image-Extraction", style="my.TButton", command=image_decode)
image_stegnography.grid(row=7, column=0,columnspan=2, pady=20)

# Button to run the String Extraction Analysis algorithm
String_analysis = ttk.Button(root.sidebar_frame,text="String Extraction", style="my.TButton", command=string_analysis)
String_analysis.grid(row=8, column=0,columnspan=2, pady=20)



root.appearance_frame = customtkinter.CTkFrame(root.sidebar_frame, width=100, corner_radius=0)
root.appearance_frame.grid(row=10, column=0, padx=20, pady=10, sticky="ew")

# Button to exit the program
style = ttk.Style()
style.configure('W.TButton', font = ('calibri', 10, 'bold'),foreground = 'red')

quitButton = ttk.Button(root.image_frame,text="Exit program", style = 'W.TButton', command=root.quit)
